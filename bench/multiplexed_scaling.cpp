...
void print_results(const char* name, std::size_t nconns, double elapsed_secs, const stats& latency)
{
...
capy::io_task<> multiplexed_bench(multiplexed_state& st, const char* name)
{
    ...
    print_results(name, st.conns.size(), std::chrono::duration<double>(t1 - t0).count(), st.latency);
