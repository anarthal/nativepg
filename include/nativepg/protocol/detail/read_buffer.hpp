...
// Always returns > 0, without UB.
// Exposed for testing.
constexpr std::size_t next_power_of_2(std::size_t n)
{
    constexpr auto sizemax = (std::numeric_limits<std::size_t>::max)();
    constexpr auto ub_boundary = (sizemax >> 1) + 1;  // bit_ceil would cause UB if the arg is > this value
    return (n <= ub_boundary) ? std::bit_ceil(n) : sizemax;
}
