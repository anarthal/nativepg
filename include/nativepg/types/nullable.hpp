...
struct nullable_type_evaluator
{
    ...
    static_assert(
        !is_nullable_ptr_v<CleanType>,
        "CRITICAL COMPILER ERROR: Smart pointers are not supported in NativePG as nullable type. Use "
        "std::optional."
    );
...
constexpr auto unwrap_or(T&& nullable_val, Fallback&& fallback_val) noexcept
{
    ...
    // Enforce that the fallback type exactly matches the underlying data type: reference_wrapper cannot
    // bind to a converted temporary, so mere convertibility is not sufficient here.
    static_assert(std::same_as<std::remove_cvref_t<Fallback>, RawType>, "Fallback type mismatch");
