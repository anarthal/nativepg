...
inline std::size_t count_significant_digits_text(const std::string_view s) noexcept
{
    ...
    auto sig_digits_view = numeric_part | std::views::filter([](const char c) { return c != '.'; }) |
...
) noexcept
{
    ...
    const auto zero_groups = static_cast<std::size_t>(std::ranges::distance(
        std::views::iota(std::size_t{0}, std::size_t{ndigits}) | std::views::reverse |
...
error_code parse_binary_numeric(const field_view& from, T& to)
{
    ...
    if (constexpr auto type_digits = static_cast<std::size_t>(std::numeric_limits<T>::digits10);
        detail::count_significant_digits_binary(ndigits, bytes.data() + 8) > type_digits)
        return client_errc::incompatible_response_length;
...
#include "nativepg/fields/field_traits_numeric.hpp"
