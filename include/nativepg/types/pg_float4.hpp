...
struct pg_type_traits<float, pg_oid_type::float4>
{
    ...
    static boost::system::error_code parse_binary(boost::span<const std::byte> bytes, value_type& val)
    {
        ...
        // FLOAT4 must be exactly 4 bytes
        ...
        // Interpret bits as IEEE-754 float
    ...
    static boost::system::error_code serialize_binary(value_type const& val, boost::span<std::byte>& bytes)
    {
        ...
        // FLOAT4 must be exactly 4 bytes for serialization
        if (bytes.size() != byte_len)
            return make_error_code(invalid_argument);

        // Bit-cast the float to a 32-bit unsigned integer
        std::uint32_t host_value = std::bit_cast<std::uint32_t>(val);
